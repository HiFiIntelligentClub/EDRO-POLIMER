...
foreach($arrObjects as $strObject)
	...
	if($strObject!=='.'&&$strObject!=='..')
		...
		/*f(empty($оГлавн->server_name_short))
			{
			if(!empty($оГлавн->server_name))
				...
				{
				echo 'No server name or server short name: '.$оГлавн->id;
		...
		$мСвойства	=(array)ЗагрузитьЭлемент::о($сГлавнаяПапка.'/'.$сБазаДанных.'/'.$сТаблица.'/belongs/Genres_control/'.$strObject);
