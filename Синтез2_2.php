...
class Синтез
	...
	private		$фОдинСтиль			=TRUE; ///// ОДИН СТИЛЬ!
	private		$ч0РасположениеКоличество	=0; //+
	...
	private function _CreateStructure($strTagLevel)
		...
		foreach($this->мСписокОбъектов as $оСтанцияЧист)
			...
			$оСтанция->strServer_type	=сПреобразовать(strtolower(str_replace(array("usac","/", " ", "audio", "application"), '', $оСтанция->server_type)),	"вКоманду");
			...
			if(empty($оСтанцияЧист->listen_url))
				{
				_Report('empty(listen_url): '.$оСтанцияЧист->server_name.'##'.$оСтанцияЧист->genre);
				continue;
				}
			if($оСтанцияЧист->listen_url=="")
				{
				_Report('listen_url=="": '.$оСтанцияЧист->server_name.'##'.$оСтанцияЧист->genre);
				continue;
				}
			if(strpos($оСтанцияЧист->server_type, 'video')!==FALSE)
				{
				_Report('video: '.$оСтанцияЧист->server_name.'##'.$оСтанцияЧист->genre);
				continue;
				}
			...
			if(фCreateListen_lnSock($оСтанцияЧист->listen_url)===FALSE)
				{
				_Report('Нет связи: '.$оСтанцияЧист->server_name.'##'.$оСтанцияЧист->genre);
				continue;
				}
	...
	private function _СоздатьСсылку($_сОбрабатываемыйОбъект, $_сЗаписываемыйОбъектРасполож)
		...
		$ч1РасположениеКоличество	=РасположениеКоличество::ч1($сЗаписываемыйОбъектРасполож, $this->фОдинСтиль, $this->ч0РасположениеКоличество);

		$сЗаписываемыйОбъект		=$сЗаписываемыйОбъектРасполож.'/'.$ч1РасположениеКоличество.'.plmr';
		if(is_file($сЗаписываемыйОбъект))
			{
			if(!unlink($сЗаписываемыйОбъект))
				{
				_Report('Не удалить '.$сЗаписываемыйОбъект);
				}
			}
